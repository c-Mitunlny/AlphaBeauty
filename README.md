...
以web为例，存在许多作业，一个作业内有很多题目，组成包含选择，判断，填空和编程。

本项目提供了所有题目类型的自动解题方案，其中，判断题和选择题一并归于“选择题”类型，采用轮询的方式遍历正确答案，完成题目。

填空题和编程题则各自采用独立的模块，逻辑相似，皆是先查看答案后，再复制答案填充到输入框中。

其中编程题脚本自动解决了答案不可手动复制的平台限制。
